...
// ---------------------------------------------------------------------------
// CCTV+AI PitchDeck.jsx — Sales Pitch Deck (21 slides · 1280×720)
// Ported from smartdata.webdemona.com/#/cctv-ai (src/pages/cctv-ai/PitchDeck.jsx)
// Design: recoloured to Repo B's "smartdata blue" palette (see C below)
// Font: Sarabun
// Content: pain-first storytelling tailored to อปท. / เทศบาล executives
// ---------------------------------------------------------------------------

// smartdata blue palette (mapped 1:1 from the source's Civic Trust palette)
const C = {
  primary: '#0071e3',
  primaryHover: '#1a80ee',
  primaryDeep: '#0056b8',
...
const IMG = asset('images/cctv-ai');
const TOTAL_SLIDES = 21;

// ---------------------------------------------------------------------------
// Slide shell — fixed 1280×720, scaled to viewport
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Slide-deck wrapper — viewport-fit scaling
// ---------------------------------------------------------------------------
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 32, height: '100%' }}>
        ...
        <div style={{ flex: 1, height: '100%', position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <div style={{
            ...
            background: `radial-gradient(ellipse at center, rgba(26, 128, 238, 0.18) 0%, rgba(6, 42, 99, 0) 70%)`,
            pointerEvents: 'none',
          }} />
          <img
      ...
      {nodes.map((n, i) => (
        <React.Fragment key={i}>
          <div style={{
            ...
            <div style={{ fontSize: 11.5, fontWeight: 700, color: n.highlight ? '#0f6e3d' : C.text, lineHeight: 1.3 }}>{n.t}</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, flex: 1, minHeight: 0 }}>
        ...
        <div style={{ background: '#FFF', border: `2px solid ${C.success}`, borderRadius: 16, padding: '16px 18px', display: 'flex', flexDirection: 'column', position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ marginTop: 'auto', paddingTop: 10, display: 'flex', gap: 6, flexWrap: 'wrap' }}>
            <Pill bg={C.successSoft} color="#0f6e3d">stable · ใช้จริง</Pill>
...
function SlideRealInstall({ num }) {
  // Repo B ships this deck's videos under public/videos/ with a flattened
  // naming scheme (overheight_main.mp4 / overheight_poster.jpg / overheight_scene.mp4)
  // instead of the source site's videos/over-height/ sub-folder. There is no
  // dedicated "scene" poster asset ported over, so the scene clip reuses the
  // main clip's poster frame (same fallback the placeholder deck already used).
  const points = [
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.6fr 1fr', gap: 20, marginTop: 14, flex: 1, minHeight: 0 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12, minHeight: 0 }}>
          ...
          <div style={{ flex: 1, minHeight: 0, borderRadius: 12, overflow: 'hidden', background: '#000', border: `1px solid ${C.surfaceSoft}`, position: 'relative' }}>
            <video
              src={VID.overheightScene}
              poster={VID.overheightScenePoster}
              controls
              muted
              playsInline
              preload="metadata"
