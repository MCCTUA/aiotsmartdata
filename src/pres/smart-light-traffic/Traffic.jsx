...
import { asset, VID } from '../../assets.js'

// ---------------------------------------------------------------------------
// Traffic.jsx — ported from smartdatapresent/src/pages/SmartTraffic.jsx
// Sales Pitch Deck (15 slides · 1280×720 · fixed-canvas scale-to-fit)
// Recoloured from the Civic Trust palette (Forest/Cream/Amber/Navy) to this
// app's Apple-blue palette — only the `C` object values were remapped below;
// slide structure/content is otherwise untouched.
// Font: Sarabun
// Audience: ผู้บริหารเมือง (นายก·ปลัด) · B2G · pain-first storytelling
//
// NOTE vs. the source file: this version is embedded inside PresViewer's own
// overlay (which already renders a 56px `.presbar` with "← กลับ"), so:
//   - `.deck-root` padding-top is 0 here (source used 56px for its own nav)
//   - the print/PDF `.deck-toolbar` (jump-to-slide + "print PDF" bar) was
//     dropped entirely — it doesn't make sense nested inside our overlay
//   - `ScrollDots` (right-edge slide nav) is kept — it's `position:fixed` and
...
const IMG = asset('images/smart_traffic')
const VIDEO_SRC = VID.dashboard
const VIDEO_POSTER = VID.dashboardPoster
...
function Eyebrow({ color, dark, alert, accent, children }) {
  const col = color || (alert ? C.alert : accent ? C.accent : dark ? '#cfe0ff' : C.primary)
  return (
    <p
      style={{
...
// ---------------------------------------------------------------------------
// Deck Styles (scroll-behavior + scale wrapper; toolbar/print removed —
// this deck is embedded inside PresViewer's own overlay chrome)
// ---------------------------------------------------------------------------
...
function DeckStyles() {
  return (
    <style>{`
      ...
      .deck-root { font-family: 'Sarabun', sans-serif; background: #6b6b6b; min-height: 100dvh; padding-top: 0; padding-bottom: 40px; max-width: 100vw; overflow-x: clip; }
    ...
    <Slide num={4} dark>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.15fr 0.85fr', gap: 24, marginTop: 16, flex: 1, alignItems: 'center' }}>
        ...
        <div style={{ display: 'grid', gridTemplateRows: 'repeat(3,1fr)', gap: 12 }}>
          {pains.map((p, i) => (
            <div key={i} style={{ background: '#16213a', borderRadius: 14, padding: '14px 18px' }}>
              <h3 style={{ fontSize: 17, fontWeight: 700, color: '#FFF', marginBottom: 4, lineHeight: 1.45 }}>{p.t}</h3>
              <p style={{ fontSize: 14, color: '#cfe0f5', lineHeight: 1.5 }}>{p.d}</p>
            </div>
          ))}
        </div>
      </div>
      <div style={{ marginTop: 12, background: 'rgba(200,120,30,0.18)', border: `1px dashed ${C.accent}`, borderRadius: 12, padding: '10px 16px', fontSize: 13, color: '#f6d9a8', lineHeight: 1.55 }}>
  ...
  return (
    <Slide num={9}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.45fr 1fr', gap: 22, marginTop: 14, flex: 1, alignItems: 'stretch' }}>
        <div style={{ width: '100%', height: '100%', borderRadius: 18, overflow: 'hidden', background: '#0d1b2e', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 10px 30px rgba(0,0,0,0.18)' }}>
...
// ---------------------------------------------------------------------------
// SLIDE 13 — DEMO VIDEO CLIP (reuses this app's existing 0518.mp4 asset)
// ---------------------------------------------------------------------------
          ...
          <div style={{ display: 'grid', gridTemplateRows: 'repeat(4,1fr)', gap: 10 }}>
            {caps.map((c, i) => (
              <div key={i} style={{ display: 'flex', gap: 12, alignItems: 'flex-start', background: 'rgba(255,255,255,0.07)', border: '1px solid rgba(255,255,255,0.14)', borderRadius: 12, padding: '12px 14px' }}>
                <div style={{ width: 38, height: 38, borderRadius: 10, background: 'rgba(200,120,30,0.25)', color: C.accent, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 19, flexShrink: 0 }}>{c.ic}</div>
